...
export default {
  ...
  methods: {
    ...
    GetSummary(sprint) {
      ...
      if (sprint.status === "DO" || sprint.status === "AC") {
        summaryArray.push([
          "Accepted Storypoints",
          statistic.sum_of_accepted_sp + " SP",
        ]);
      } else {
        summaryArray.push([
          "Done Storypoints",
          statistic.sum_of_done_sp + " SP",
        ]);
        summaryArray.push([
          "Not Done Storypoints",
          statistic.sum_of_sp - statistic.sum_of_done_sp + " SP",
        ]);
      }
      // Tasks
      ...
      if (sprint.status === "DO" || sprint.status === "AC") {
        ...
        summaryArray.push([
          "Not Done Tasks",
          statistic.sum_of_tasks - statistic.sum_of_done_tasks,
        ]);
      }
      return summaryArray;
    },
    ...
    GetDetails(sprint) {
      ...
      tasks.forEach((task) => {
        detailArray.push([
          ...
          task.description,
          task.storypoints,
          this.GetNamedStatus(task.status),
        ]);
      });
