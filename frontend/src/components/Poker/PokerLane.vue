  ...
  <v-card-title class="white--text">
    ...
    <v-card-text class="overflow-y-auto" style="height: calc(95vh - 355px)">
      ...
      <v-row no-gutters align="center">
        <v-col cols="6">
          <v-checkbox label="Open End Vote" v-model="openEndVote" value color="link" dark></v-checkbox>
        </v-col>
        <v-col cols="6">
          <v-menu
            ref="voteDateMenu"
    ...
    <v-card-actions>
      ...
      <v-spacer></v-spacer>
      <v-btn outlined color="link" :disabled="!allowAsyncPoker" @click="asyncPokerDialog=true">Poker Async</v-btn>
    </v-card-actions>
</v-card>
  ...
  computed: {
    ...
    allowAsyncPoker(){
      var allowed = false;
        ...
        else if(this.cardsToVote.length > 0 && !this.openEndVote && this.endVoteDate !== null){
          allowed = true;
        }
        else{
          allowed = false;
  ...
  methods: {
    ...
    addPoker(e){
      ...
      if(this.cardsToVote.findIndex(item => item.cardId == taskId) == -1){
        this.cardsToVote.push({cardName: taskName , cardId: taskId, cardStorypoints: taskStorypoints});
      }
      else{
    ...
    removePoker(vote){
      this.cardsToVote = this.cardsToVote.filter(item => item.cardId !== vote.cardId);
    },
