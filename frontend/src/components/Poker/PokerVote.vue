<template>
  <v-expansion-panel class="primary" @click="selectPokerVote()">
      ...
      <v-row no-gutters>
        ...
        <v-col cols="1">
          <v-tooltip top>
            ...
            <span v-if="voteStatus === 'VOTED'"
              >Your storypoints have been submitted</span
            >
    ...
    <v-expansion-panel-content>
      <v-row no-gutters justify="start">
        <v-col>
          ...
          <v-row class="ma-2">
            <v-btn text color="white" outlined @click="showTaskDetails()">
              More Details
            </v-btn>
          </v-row>
        </v-col>
...
export default {
  ...
  computed: {
    ...
    votes() {
      ...
      return this.voteByIds({
        pokerVoteId: this.pokerVote.id,
        userId: this.userId,
      });
    },
    voteStatus() {
