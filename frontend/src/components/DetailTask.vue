    ...
    dark>
        <v-container>
            ...
            <v-tabs-items v-model="tab" background-color="secondary" color="secondary">
                <v-tab-item>
                    ...
                    <v-card v-if="!this.selectedTask.visableCreate" flat dark color="secondary" tile>
                        ...
                        <div v-for="avatar in allAssignedUsers" 
            ...
            <div>
                ...
                <v-btn v-if="this.selectedTask.visableCreate" color="link" :disabled="!isFormValid" text @click="addTask()">Create</v-btn>
                <v-btn v-if="!this.selectedTask.visableCreate" color="error" text absolute right @click="deleteDialog = true">
...
export default {
    ...
    computed: {
        ...
        allAssignedUsers(){
            return (this.plattformUsersByIdArray(this.localTask.assigned_users) && this.plattformUsersByIdArray(this.localTask.assigned_users).length > 0)
            ? this.plattformUsersByIdArray(this.localTask.assigned_users).sort((a, b) => a.username.localeCompare(b.alt))
            : null            
        },
