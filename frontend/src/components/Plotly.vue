...
export default {
    ...
    methods: {
            ...
            if(this.limitAxis) {
                ...
                while(this.internalLayout[name]) {
                    if(evt[name + '.range[0]'] && evt[name + '.range[1]']) {
                        ...
                        if(this.internalLayout[name].type && this.internalLayout[name].type == 'date') {
                        ...
                        this.data.forEach(trace => {
                            if((trace.xaxis && trace.xaxis == nameShort) || (idx == 1 && !trace.xaxis)) {
        ...
        async saveXlsx() {
            ...
            this.data.forEach(pd => {
                if(pd.exportName) {
                    pd.x.forEach((x, idx) => {
                        ...
                        data[x][pd.exportName] = pd.y[idx] instanceof Date ? pd.y[idx].getTime() : pd.y[idx]
                    })
                }
            })
            ...
            let sheet = wb.addWorksheet('export', { views: [{ state: 'frozen', ySplit: 1 }] })
