    ...
    <v-card-text>
      <v-row align="center">
        <v-col cols="12">
          ...
          <span class="text-body-2 white--text">Drop task here to vote them</span>
          </v-sheet>
        </v-col>
      </v-row>
      <v-row>
        <v-list two-line class="transparent">
          <v-subheader class="white--text text-body-1">Tasks to vote</v-subheader>
          <v-list-item v-for="item in toVote" :key="item.name">
          <v-list-item-content>
          ...
          <v-list-item-action>
            <v-btn icon>
              <v-icon color="white" @click="removePoker(item)">mdi-delete</v-icon>
            </v-btn>
          </v-list-item-action>
          </v-list-item>
        </v-list>
...
export default {
  ...
  methods: {
    ...
    addPoker(e){
      ...
      this.toVote.push({name: taskName, id: taskId, storypoints: taskStorypoints})

    },
    removePoker(vote){
      console.log(vote)
      console.log(this.toVote)
      this.toVote = this.toVote.filter(item => item.id !== vote.id);
    }
